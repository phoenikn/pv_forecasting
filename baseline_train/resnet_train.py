...
RESNET_MODEL_PATH = "resnet_{}_interval.pth".format(DATASET_ARG["interval"]-5)
...
def implement():
    ...
    model.conv1 = nn.Conv2d(90, 64, kernel_size=(7, 7))
    ...
    if (not os.path.exists(RESNET_MODEL_PATH)) or (os.path.exists(RESNET_MODEL_PATH) and TRAIN_AGAIN):
        ...
        for epoch in range(EPOCH):
            ...
            if avg_val_loss < best_loss:
                torch.save(model.state_dict(), "{}_best_interval{}.pth".format("RESNET", DATASET_ARG["interval"]-5))
                best_loss = avg_val_loss

            records = records.append({"epoch": epoch + 1, "train_loss": avg_train_loss, "val_loss": avg_val_loss},
                                     ignore_index=True)

        records.to_csv("training_records_{}epochs_RES_loss.csv".format(EPOCH), index=False)
