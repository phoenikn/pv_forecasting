...
def implement():
    ...
    model = SCNN()
    if TRAIN_AGAIN:
        model.load_state_dict(torch.load(SCNN_MODEL_PATH, map_location=device))

    criterion = nn.MSELoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)
    model.to(device)

    if (not os.path.exists(SCNN_MODEL_PATH)) or (os.path.exists(SCNN_MODEL_PATH) and TRAIN_AGAIN):
        ...
        for epoch in range(EPOCH):
            ...
            if avg_val_loss < best_loss:
                torch.save(model.state_dict(), "{}_best.pth".format("SCNN"))
                best_loss = avg_val_loss

            records = records.append({"epoch": epoch + 1, "train_loss": avg_train_loss, "val_loss": avg_val_loss},
                                     ignore_index=True)

        records.to_csv("training_records_{}epochs_SCNN_loss.csv".format(EPOCH), index=False)
        torch.save(model.state_dict(), SCNN_MODEL_PATH)
        print("Finish!")
    else:
